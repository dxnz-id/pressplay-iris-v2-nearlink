...
async function testDiscovery() {
    ...
    const filtered = devices.filter(d => d.vendorId === VID && PIDS.includes(d.productId));
    ...
    console.log(`Found ${filtered.length} matching device(s). Looking for Management Interface (UsagePage: 0xFFA0)...`);

    for (const d of filtered) {
        console.log(`\n--- Device at ${d.path} ---`);
        console.log(` - Interface: ${d.interface}, UsagePage: 0x${d.usagePage.toString(16)}, Usage: ${d.usage}`);
        ...
        try {
            ...
            // Setup Listener
            device.on('data', (data) => {
                console.log(` - [RESPONSE] Received (hex): ${Buffer.from(data).toString('hex')}`);
                if (data[0] === 0x0D) {
            ...
            console.log(` - Sending 'Get Info' command (0x13) wrapped in Report ID 0x0B...`);
